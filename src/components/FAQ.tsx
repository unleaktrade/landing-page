...
const faqData: FAQItem[] = [
  {
    ...
    answer: "UnleakTrade is a private **OTC (Over-The-Counter)** auction platform on **Solana** for trading any **SPL (Solana Program Library) token** (listed or unlisted) via private auctions, with **ZK (Zero-Knowledge)** verified liquidity and trustless, fully on-chain settlement."
  },
  {
    category: "Platform Basics",
    question: "Why use this OTC instead of another OTC venue or an AMM?",
    answer: "Most venues either execute in public (so size, timing, and intent leak) or rely on trust-heavy coordination. UnleakTrade's core differentiator is that it encodes the entire **RFQ (Request For Quote)** lifecycle into a verifiable on-chain state machine, and uses bonds + timeouts to enforce completion and penalize griefing. This matters for whales because the cost of \"leakage\" is usually not the fee: it's adverse price movement and being gamed."
  },
  {
    category: "Platform Basics",
    question: "Why not an AMM?",
    answer: "An AMM (**Automated Market Maker)** prices against a public pool and your trade is executed against a curve; large trades tend to pay explicit slippage and also reveal execution to the market. UnleakTrade instead runs a private auction where takers compete to fill a maker's request, and only the final settlement needs to hit the chain. That's typically better for large size because you're seeking a negotiated/competitive fill rather than pushing a pool price."
  },
  {
    category: "Platform Basics",
    question: "What's a DEX?",
    answer: "A DEX (**Decentralized Exchange)** is an on-chain exchange - often an AMM or an order book - where orders/swaps are executed via smart contracts. UnleakTrade is still on-chain for settlement, but it's not a \"public order book DEX\" pattern, it is an OTC/RFQ auction flow with commit/reveal and bonding to minimize information leakage and enforce fair behavior."
  },
  {
    category: "Core Concepts",
    question: "What's an RFQ (Request For Quote) on UnleakTrade?",
    answer: "An RFQ is an on-chain object created by the **maker** that defines the trade request (tokens involved), the bonding requirement, and the timing windows. It is uniquely identified by `(maker, uuid)` and stored in a dedicated on-chain RFQ account (a **PDA = Program Derived Address**)."
  },
  {
    category: "Roles & Participants",
    question: "Who is the maker on UnleakTrade?",
    answer: "On UnleakTrade, the maker is the party who **initiates** the RFQ and later **selects** the winning taker. The maker creates the RFQ draft, publishes it, and after the auction they must fund their side of the settlement within the funding time window."
  },
  {
    category: "Roles & Participants",
    question: "Who is the taker on UnleakTrade?",
    answer: "On UnleakTrade, takers are bidders: they respond to a maker's RFQ by submitting bids/quotes during the commit/reveal auction. If a taker is selected, they must fund their side of the settlement (and pay the protocol fee, in USDC) within the funding time window."
  },
  {
    category: "Core Concepts",
    question: "What's a Quote vs a Bid vs an Auction?",
    answer: "A quote (or bid) is the taker's proposed execution terms for the maker's RFQ (the price they're willing to trade at for the specified pair/size). The auction is the competitive process where multiple takers submit quotes and the maker selects the winning one. UnleakTrade implements this with a commit/reveal flow: takers first commit a hash of their quote, then later reveal the details for validation."
  },
  {
    category: "Technical Mechanics",
    question: "What is commit/reveal and why does it matter?",
    answer: "Commit/reveal is a two-step auction design used to reduce copying and information leakage. In the **commit** phase, takers submit a hashed commitment (so the content isn't revealed). In the **reveal** phase, takers reveal the actual bid details and the program validates that the revealed quote matches the prior commitment. This protects bidders from having their quote instantly mirrored and helps keep the process fair for size."
  },
  {
    category: "Technical Mechanics",
    question: "Is it fully on-chain?",
    answer: "The settlement engine is an on-chain Solana program that enforces the RFQ lifecycle, bond accounting, state transitions, and atomic settlement. There is also an off-chain component (\"Liquidity Guard\") that performs validation and generates attestations and ZK proofs used during the process, but the final enforcement and settlement happen on-chain."
  },
  {
    category: "Technical Mechanics",
    question: "What's ZK (Zero-Knowledge) doing here?",
    answer: "ZK (Zero-Knowledge) is used so a participant can prove something (for example, eligibility/solvency constraints required by the protocol flow) without publicly revealing sensitive details. In UnleakTrade's flow, takers generate a ZK proof via a REST API and submit it with their commitment; invalid proofs are rejected."
  },
  {
    category: "Technical Mechanics",
    question: "What is the \"Liquidity Guard\"? (and what does \"REST API\" mean?)",
    answer: "Liquidity Guard is an off-chain microservice that (1) verifies liquidity/solvency before commitment, (2) generates signed attestations for makers and takers, and (3) feeds validated actions to the on-chain Settlement Engine.\n\nA **REST API (Representational State Transfer Application Programming Interface)** is simply an HTTP service interface; here it's used to request and receive ZK proof material needed for the on-chain commit step."
  },
  {
    category: "Economics & Incentives",
    question: "What are \"fees\" on UnleakTrade?",
    answer: "A protocol fee is paid during funding/settlement: in the reference sequence, the selected taker deposits their settlement asset and pays a fee in **USDC (USD Coin)**, which is transferred to the UnleakTrade treasury. The exact fee amount/rate is a protocol parameter."
  },
  {
    category: "Economics & Incentives",
    question: "What are \"bonds\" and why do they exist?",
    answer: "A bond is a USDC amount posted by *each participant* to make griefing expensive and force timely completion. Bonds are held in an RFQ-owned USDC token account (an **ATA = Associated Token Account**) and are returned on successful settlement. If someone fails to do their required step in time, their bond can be slashed and redistributed. Counterparties **have skin-in-the-game**."
  },
  {
    category: "Economics & Incentives",
...
const renderText = (text: string) => {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return <code key={i} className="px-1.5 py-0.5 bg-white/5 text-purple-300 rounded text-sm font-mono">{part.slice(1, -1)}</code>;
    }
    return part;
  });
};
  ...
  return (
    <section className="relative py-32 px-6 overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto">
        ...
        >
          <p className="text-white/40 mb-4">Questions & Answers</p>
          <h2 className="text-white mb-6">Everything you need to know.</h2>
          <p className="text-white/60 max-w-2xl mx-auto">
            Deep technical documentation on protocol mechanics, auction flow, and settlement architecture.
          </p>
        </motion.div>
            ...
            return (
              ...
              >
                <div className="group relative">
                  ...
                  <div className={`absolute inset-0 rounded-xl bg-gradient-to-r from-purple-500/10 to-cyan-500/10 opacity-0 transition-opacity duration-300 ${
                    isOpen ? 'opacity-100' : 'group-hover:opacity-50'
                  }`} />
                  
                  <div className={`relative bg-white/[0.02] border rounded-xl transition-all duration-300 ${
                    isOpen ? 'border-white/20' : 'border-white/10 group-hover:border-white/15'
                  }`}>
                    {/* Question button */}
                    <button
                    ...
                    >
                      <div className="flex-1">
                        ...
                        {!isOpen && (
                          <p className="text-white/30 text-sm mt-1">{item.category}</p>
                        )}
                      </div>
                      <ChevronDown 
                        ...
                        >
                          <div className="px-6 pb-4 pt-0 border-t border-white/5">
                            <div className="pt-4 space-y-3">
                              <p className="text-purple-400/60 text-sm">{item.category}</p>
